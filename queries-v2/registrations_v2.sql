...
WITH base AS (
  ...
  WHERE DATE(ds) >= '2025-01-01' AND site_id IN ('MLB','MLM','MLC','MLA')
)
...
SELECT 'week',
  FORMAT_DATE('%Y-%m-%d', DATE_ADD(DATE '2026-06-15',
      INTERVAL DIV(DATE_DIFF(ds, DATE '2026-06-15', DAY), 7) * 7 DAY)),
  site_id, origen, origen_grouped, COUNT(*),
  NULL, NULL
FROM base
WHERE DATE_DIFF(ds, DATE '2026-06-15', DAY) BETWEEN 0 AND 83
GROUP BY 1,2,3,4,5
ORDER BY 1,3,2,4
