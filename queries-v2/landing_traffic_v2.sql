...
GROUP BY 1,2,3,4
UNION ALL
SELECT 'mtd_curr', '${D.CUR}', site, origen, SUM(visitas), SUM(qty_users), SUM(qty_users_loggedin)
FROM diario WHERE ds BETWEEN '${D.CUR}' AND '${D.YEST}' GROUP BY 1,2,3,4
UNION ALL
SELECT 'mtd_prev', '${D.PREV}', site, origen, SUM(visitas), SUM(qty_users), SUM(qty_users_loggedin)
