...
$sqlMap["act_source"] = d @'
WITH reg_chan AS (
  SELECT SAFE_CAST(user_id AS INT64) AS user_id, site_id,
    DATE_TRUNC(DATE(ds), MONTH) AS mes_reg,
    MAX(CASE WHEN UPPER(origen_grouped) = 'POM' THEN 'POM' ELSE 'Direct' END) AS canal
  FROM `meli-bi-data.SBOX_AFILIADOSCOREDATA.AFFILIATE_REGISTRATION_CHANNEL`
  WHERE DATE(ds) >= '${D.HIST}'
    AND DATE_TRUNC(DATE(ds), MONTH) < '${D.CUR}'
    AND site_id IN ('MLB','MLM','MLC','MLA')
    AND user_id IS NOT NULL
  GROUP BY user_id, site_id, DATE_TRUNC(DATE(ds), MONTH)
),
first_sale AS (
  SELECT SIT_SITE_ID, AFFILIATE_ID,
    DATE_TRUNC(MIN(ORD_CREATED_DT), MONTH) AS mes_primera_venta
...
$cfg = '{"skill_version":"3.6.4","doc_id":"01KRE46H4452DPPVSYM5BKXJ14"}'
...
"@
    ...
    if ($pacingRows.Count -gt 100) {
        ...
        # Upload a Grid
        $pacingCfg = "{`"skill_version`":`"3.6.4`",`"doc_id`":`"$pacingDocId`"}"
