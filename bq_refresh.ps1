...
earners AS (SELECT SIT_SITE_ID, month AS mes, AFFILIATE_ID AS affiliate_id FROM ventas WHERE month >= '${D.HIST}'),
eb AS (
...
# Las 5 queries de link_gen se unificaron en una sola (escaneaba 1.081 GB, ahora 349).
# Devuelve _q + r: aca se reparte por _q y cada grupo pasa por Parse-BQResult, asi
# hereda el pasaje a minusculas de las claves y la conversion numerica de siempre.
function Split-ByQ([string]$jsonStr) {
    $out = @{}
    if (-not $jsonStr -or $jsonStr -eq '[]' -or $jsonStr -eq '') { return $out }
    try {
        $rows = $jsonStr | ConvertFrom-Json
        if ($rows -isnot [System.Array]) { $rows = @($rows) }
        $b = @{}
        foreach ($row in $rows) {
            $q = $row._q
            if (-not $b.ContainsKey($q)) { $b[$q] = New-Object System.Collections.Generic.List[string] }
            $b[$q].Add($row.r)
        }
        foreach ($q in $b.Keys) { $out[$q] = Parse-BQResult ('[' + ($b[$q] -join ',') + ']') }
    } catch {
        Write-Host "  Split-ByQ error ($($_.Exception.Message))" -ForegroundColor Yellow
    }
    return $out
}
...
$lgAll = Split-ByQ $rawResults["link_gen_all"].json
...
$data = [ordered]@{
    ...
    link_gen_monthly   = @($lgAll["link_gen_monthly"])
    link_gen_daily     = @($lgAll["link_gen_daily"])
    link_gen_mtd_comp  = @($lgAll["link_gen_mtd_comp"])
    link_gen_by_segment= @($lgAll["link_gen_by_segment"])
    link_gen_earnings_by_status = @($lgAll["link_gen_earnings_by_status"])
